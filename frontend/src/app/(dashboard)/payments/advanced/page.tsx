...
export default function AdvancedPaymentsPage() {
  ...
  const clearOrBounce = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json()).message || 'Update failed');
      showMsg(clearForm.action === 'clear' ? 'Cleared successfully.' : 'Marked as bounced.');
      setClearModal(null);
      loadInstruments();
    } catch (e: unknown) { showMsg(e instanceof Error ? e.message : 'Operation failed', true); }
    setLoading(false);
  ...
  const parseAndImportCSV = async () => {
    ...
    try {
      ...
      showMsg(`${rows.length} statements imported.`);
  ...
  const generateQR = async () => {
    if (!selectedUpiStudent) { showMsg('Select a student first.', true); return; }
    setLoading(true);
    try {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Advanced Payment Management</h1>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 flex flex-wrap gap-3 items-end">
            ...
            <div>
              ...
              <select value={instFilter.status} onChange={e => setInstFilter({ ...instFilter, status: e.target.value })}
                ...
                {['pending','cleared','bounced'].map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            ...
            <table className="w-full text-sm min-w-max">
              <thead>
                <tr className="bg-gray-50/80 text-gray-600 font-medium border-b border-gray-200">
                  ...
                  <th className="px-4 py-3 text-left">Instrument No</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  ...
                  <tr><td colSpan={9} className="px-4 py-8 text-center text-gray-400">No instruments found.</td></tr>
                ) : instruments.map(inst => (
                  <tr key={String(inst.id)} className="hover:bg-gray-50/50">
                    <td className="px-4 py-3 font-mono text-xs text-gray-700">{String(inst.receipt_no || '')}</td>
                    <td className="px-4 py-3 font-medium text-gray-800">{String(inst.student_name || '')}</td>
                    <td className="px-4 py-3 text-right font-semibold">₹{Number(inst.amount).toLocaleString('en-IN')}</td>
                    <td className="px-4 py-3"><span className="px-2 py-1 bg-purple-50 text-[#6c5ce7] text-xs rounded-lg">{String(inst.instrument_type || inst.payment_mode || '')}</span></td>
                    <td className="px-4 py-3 text-gray-600 font-mono text-xs">{String(inst.instrument_number || '-')}</td>
                    <td className="px-4 py-3 text-gray-600">{String(inst.bank_name || '-')}</td>
                    <td className="px-4 py-3 text-gray-600">{String(inst.instrument_date || inst.payment_date || '')}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 text-xs font-medium rounded-lg ${inst.status === 'cleared' ? 'bg-emerald-50 text-emerald-700' : inst.status === 'bounced' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button onClick={() => { setUpdateModal(inst); setUpdateForm({ instrument_type: String(inst.instrument_type || ''), instrument_number: String(inst.instrument_number || ''), bank_name: String(inst.bank_name || ''), instrument_date: String(inst.instrument_date || '') }); }}
                          className="px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">Update</button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/80 text-gray-600 font-medium border-b border-gray-200">
                ...
                <th className="px-4 py-3 text-left">Bank</th>
                <th className="px-4 py-3 text-left">Bounce Date</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-4 py-8 text-center text-gray-400">No bounced cheques found.</td></tr>
              ) : bouncedList.map(item => (
                <tr key={String(item.id)} className="hover:bg-gray-50/50">
                  <td className="px-4 py-3 font-mono text-xs">{String(item.receipt_no || '')}</td>
                  <td className="px-4 py-3 font-medium text-gray-800">{String(item.student_name || '')}</td>
                  <td className="px-4 py-3 text-right font-semibold">₹{Number(item.amount).toLocaleString('en-IN')}</td>
                  <td className="px-4 py-3 font-mono text-xs text-gray-600">{String(item.instrument_number || '')}</td>
                  <td className="px-4 py-3 text-gray-600">{String(item.bank_name || '')}</td>
                  <td className="px-4 py-3 text-gray-600">{String(item.bounce_date || item.instrument_date || '')}</td>
                  <td className="px-4 py-3 text-right text-red-600 font-medium">₹{Number(item.bounce_penalty || 0).toLocaleString('en-IN')}</td>
                  <td className="px-4 py-3"><span className="px-2 py-1 bg-red-50 text-red-700 text-xs font-medium rounded-lg">Bounced</span></td>
                  <td className="px-4 py-3">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {unreconciled.length === 0 ? (
                  <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">No unreconciled entries.</td></tr>
                ) : unreconciled.map(entry => (
                  <tr key={String(entry.id)} className="hover:bg-gray-50/50">
                    <td className="px-4 py-3 text-gray-600">{String(entry.date || '')}</td>
                    <td className="px-4 py-3 text-gray-800">{String(entry.description || '')}</td>
                    <td className="px-4 py-3 text-right text-emerald-700 font-semibold">₹{Number(entry.credit).toLocaleString('en-IN')}</td>
                    <td className="px-4 py-3 font-mono text-xs text-gray-500">{String(entry.reference || '')}</td>
                    <td className="px-4 py-3">
      ...
      {activeTab === 'UPI QR' && (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4 max-w-lg">
            <h2 className="font-semibold text-gray-800">Generate UPI QR Code</h2>
                ...
                <div className="absolute top-full left-0 right-0 z-10 bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-40 overflow-y-auto">
                  {upiSearchResults.map(s => (
                    <button key={String(s.id)} onClick={() => { setSelectedUpiStudent(s); setUpiSearch(String(s.name || '')); setUpiSearchResults([]); }}
                      ...
                      <span className="font-medium">{String(s.name || '')}</span> <span className="text-gray-500 text-xs">— {String(s.admission_no || '')} {String(s.class_name || '')}</span>
                    </button>
                  ))}
                </div>
              )}
            ...
            {selectedUpiStudent && (
              <div className="bg-purple-50 rounded-xl p-3 text-sm">
                ...
                <span className="text-gray-500 text-xs ml-2">({String(selectedUpiStudent.admission_no || '')})</span>
              </div>
            )}
            ...
            {generatedQR && (
              <div className="border border-gray-200 rounded-xl p-4 text-center space-y-3">
                ...
                {generatedQR.qr_image_url ? (
                  ...
                  <div className="w-48 h-48 mx-auto bg-gray-100 rounded-xl flex items-center justify-center text-gray-400 text-sm">QR Image</div>
                )}
                <p className="text-xs text-gray-500">UPI ID: {String(generatedQR.upi_id || '')}</p>
                <p className="text-xs text-gray-500">Amount: ₹{Number(generatedQR.amount || 0).toLocaleString('en-IN')}</p>
          ...
          {recentQRs.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-gray-100">
                <h2 className="font-semibold text-gray-800">Recently Generated QR Codes</h2>
              </div>
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50/80 text-gray-600 font-medium border-b border-gray-200">
                ...
                <tbody className="divide-y divide-gray-100">
                  {recentQRs.map((qr, i) => (
                    <tr key={i} className="hover:bg-gray-50/50">
                      <td className="px-4 py-3 font-medium text-gray-800">{String(qr.student_name || '')}</td>
                      <td className="px-4 py-3 text-right font-semibold">₹{Number(qr.amount || 0).toLocaleString('en-IN')}</td>
                      <td className="px-4 py-3 text-gray-600 font-mono text-xs">{String(qr.upi_id || '')}</td>
                      <td className="px-4 py-3 text-gray-500 text-xs">{String(qr.created_at || '')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {updateModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
            <h2 className="font-bold text-gray-900">Update Instrument Details</h2>
            {([['instrument_type','Instrument Type'],['instrument_number','Instrument Number'],['bank_name','Bank Name'],['instrument_date','Instrument Date']] as [string,string][]).map(([k, label]) => (
              <div key={k}>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setUpdateModal(null)} className="px-4 py-2 text-sm bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200">Cancel</button>
      ...
      {clearModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            <h2 className="font-bold text-gray-900">Clear / Bounce Instrument</h2>
            <div className="flex gap-3">
              {['clear','bounce'].map(a => (
                <button key={a} onClick={() => setClearForm({ ...clearForm, action: a })}
                  className={`px-4 py-2 text-sm font-medium rounded-xl border ${clearForm.action === a ? 'bg-[#6c5ce7] text-white border-[#6c5ce7]' : 'bg-white text-gray-700 border-gray-200 hover:border-gray-300'}`}>
            ...
            {clearForm.action === 'clear' ? (
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Bounce Penalty (₹)</label>
                <input type="number" value={clearForm.bounce_penalty} onChange={e => setClearForm({ ...clearForm, bounce_penalty: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#6c5ce7]/20 focus:border-[#6c5ce7]" />
              </div>
            )}
            <div className="flex justify-end gap-2">
              <button onClick={() => setClearModal(null)} className="px-4 py-2 text-sm bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200">Cancel</button>
      ...
      {importModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 space-y-4">
            <h2 className="font-bold text-gray-900">Import Bank Statement (CSV)</h2>
            <p className="text-xs text-gray-500">Format: date, description, credit, debit, balance, reference (one row per line, first row is header)</p>
            <textarea rows={10} value={csvText} onChange={e => setCsvText(e.target.value)}
              ...
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm font-mono focus:ring-2 focus:ring-[#6c5ce7]/20 focus:border-[#6c5ce7] resize-none" />
            <div className="flex justify-end gap-2">
              <button onClick={() => setImportModal(false)} className="px-4 py-2 text-sm bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200">Cancel</button>
      ...
      {matchModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 space-y-4">
            <h2 className="font-bold text-gray-900">Match Entry — {String(matchModal.description || '')}</h2>
            <p className="text-sm text-gray-600">Amount: <span className="font-semibold text-emerald-700">₹{Number(matchModal.credit).toLocaleString('en-IN')}</span></p>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Search Fee Payment</label>
            ...
            {matchResults.length > 0 && (
              <div className="border border-gray-200 rounded-lg max-h-48 overflow-y-auto">
                {matchResults.map(p => (
                  <div key={String(p.id)} className="flex items-center justify-between px-3 py-2 border-b last:border-0 hover:bg-gray-50">
                    <div>
                      <p className="text-sm font-medium">{String(p.student_name || '')}</p>
                      <p className="text-xs text-gray-500">{String(p.receipt_no || '')} — {String(p.payment_date || '')}</p>
                    </div>
            ...
            <div className="flex justify-end">
              <button onClick={() => setMatchModal(null)} className="px-4 py-2 text-sm bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200">Cancel</button>
            </div>
          </div>
        </div>
      )}
